...
class EnhancedDocumentProcessor:
    ...
    def _find_all_image_instances_on_page(self, page: fitz.Page, xref: int,
        ...
        try:
            ...
            try:
                # Use a more basic approach for getting image positions
        ...
        for image_instance in self.image_instances:
            try:
                ...
                try:
                    ...
                    logger.error(f"Failed to extract image data for {image_instance.instance_id}: {img_extract_error}")
                    continue
    ...
    def _process_docx_images(self, docx_zip: zipfile.ZipFile, root) -> int:
        ...
        try:
            ...
            try:
                ...
                logger.info("No image relationships found in DOCX")
                return 0
    ...
    # Additional helper methods that might be missing from your original code
    # Add these if your code references them but they're not implemented
    ...
    try:
        ...
        # Handle direct invocation
            ...
            try:
                ...
                logger.error(f"Failed to download file from S3: {download_error}")
                return {
                    'statusCode': 500,
                    'body': json.dumps({
                        'success': False,
                        'error': 'file_download_failed',
                        'message': f'Failed to download file from S3: {str(download_error)}',
                        'request_id': request_id
                    }),
                    'headers': {
                        'Content-Type': 'application/json',
